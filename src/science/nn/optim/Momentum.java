...
public class Momentum implements Optimizer{
    ...
    public Momentum(double lernrate){
        this.lernrate = lernrate;
    }

    public Momentum(double lernrate, double beta){
        this.lernrate = lernrate;
        this.beta = beta;
    }

    public Momentum(double lernrate, double beta, double epsilon){
        this.lernrate = lernrate;
        this.beta = beta;
    ...
    public void optimize(Layer layer) {
        ...
        for (Neuron neuron : layer.getShape()){
            ...
            for (Neuron successor : weights.keySet()){
                ...
                sdwPrev = beta * sdwPrev + (1 - beta) * delta;
                delta = sdwPrev;
